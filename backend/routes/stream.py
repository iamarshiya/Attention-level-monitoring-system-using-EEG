        ...
        while True:
            try:
                ...
                if hasattr(app_state, 'uploaded_df') and app_state.uploaded_df is not None:
                    ...
                    if not eeg_cols:
                        # Fallback: use numeric columns except Timestamp/label
                        num_cols = df.select_dtypes(include=[np.number]).columns.tolist()
                        eeg_cols = [c for c in num_cols if c.lower() not in ['timestamp', 'label', 'attention']]
                        if not eeg_cols:
                    ...
                    time_series = window_df[eeg_cols[0]].fillna(0).tolist()
                    
                    if len(time_series) < 256:
                        pad = [time_series[0]] * (256 - len(time_series))
                        time_series = pad + time_series
                        
                    window = np.array(time_series, dtype=float)
                    ...
                    # Force Random Forest path — CNN needs retraining before live use
                    prediction = model_instance.predict(features, stft_2d=None)
                    ...
                    # DL Confidence with slight drift
                    ...
                    # Timestamp Parsing — use pandas Series indexing, not dict.get()
                    ts = "00:00:00"
                    if 'Timestamp' in row.index:
                        try:
                            ts_val = str(row['Timestamp'])
                            ts = ts_val.split(" ")[1][:8] if len(ts_val) > 10 else ts_val
                        except Exception:
                            ts = "00:00:00"
                    ...
                    # User has not uploaded data yet
                ...
                import traceback
                traceback.print_exc()
                print(f"Stream processing error (keeping connection alive): {e}")
                await asyncio.sleep(1.0)  # Brief pause then continue the loop
